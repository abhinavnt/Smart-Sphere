...
document.addEventListener('DOMContentLoaded', () => {
    ...
    cartItems.forEach(item => {
        ...
        const itemId = item.querySelector('.remove-item').getAttribute('data-id'); // Get item ID
        ...
        increaseButton.addEventListener('click', async () => {
            ...
            if (currentQuantity <= 5 && currentQuantity <= availableStock) {
                ...
                updateCartTotal();
                updateCartItemQuantity(itemId, currentQuantity); // Update backend
        ...
        quantityInput.addEventListener('input', () => {
            ...
            if (isNaN(currentQuantity) || currentQuantity < 1) {
                ...
                Swal.fire('Maximum Limit Reached', 'You cannot add more than 5 items!', 'warning');
                quantityInput.value = 5; // Reset to 5 if exceeded
                currentQuantity = 5;
            }
    ...
    async function checkStock(productId) {
        try {
            ...
            return response.data.availableStock; // Assuming it returns availableStock
        } catch (error) {
            console.error('Error checking stock:', error);
            return 0; // Default response on error
        }
    }
...
//             btnUp.disabled = value >= 5; // Disable up button if value is 5 or more
//             btnDown.disabled = value <= 1; // Disable down button if value is 1 or less
//         }

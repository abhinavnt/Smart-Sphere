  ...
  searchInput.addEventListener("input", () => {
    ...
    productCards.forEach((card) => {
      const productName = card
        .querySelector(".card-title strong")
...
function checkStock(productId) {
  return axios
    .post(`/cart/check-stock/${productId}`)
...
function addToCart(product) {
  ...
  return axios
    .post(`/cart/add/${window.userId}`, cartItem)
    .then((response) => {
...
document.addEventListener("DOMContentLoaded", () => {
  const productCards = Array.from(
    document.querySelectorAll(".col-sm-6.col-md-4.col-lg-3")
  );
  const sortLinks = document.querySelectorAll(".filter-link[data-sort]");
    ...
    switch (sortBy) {
      ...
      case "priceLowToHigh":
        sortedProducts = products.sort((a, b) => {
          return (
            parseFloat(
              a.querySelector(".card-text").innerText.replace("₹", "")
            ) -
      ...
      case "priceHighToLow":
        sortedProducts = products.sort((a, b) => {
          return (
            parseFloat(
              b.querySelector(".card-text").innerText.replace("₹", "")
            ) -
